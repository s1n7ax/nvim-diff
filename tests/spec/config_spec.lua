...
describe("config", function()
  ...
  describe("validation", function()
    ...
    it("takes a key or false for a keymap, never true", function()
      config.setup({ keymaps = { panel = { select = "o", refresh = false } } })
      expect.eq("o", config.get().keymaps.panel.select)
      expect.eq(false, config.get().keymaps.panel.refresh)
      expect.eq("<Tab>", config.get().keymaps.view.next_file)
      local err = expect.errors(function()
        config.setup({ keymaps = { view = { next_file = true } } })
      end)
      expect.matches("`keymaps%.view%.next_file`: expected a key or false, got true", err)
    end)

    it("checks the panel options", function()
      local err = expect.errors(function()
        config.setup({ panel = { listing = "list", width = 0 } })
      end)
      expect.matches("`panel%.listing`: expected one of tree, flat", err)
      expect.matches("`panel%.width`: expected at least 1", err)
    end)
