...
- [x] implement: range compare and line history — needs: file history panel — [result](#result-implement-range-compare-and-line-history)
...
### result: implement: GitHub client

Merged to `main` in `4c2778a` (branch `worktree-agent-abf51689451e5557e`, commit `f2dd482`), run in parallel with the two steps below. `make check` on the branch: 432/432 (39 new). Clean merge, no overlap with the other two steps in this wave.

Built `github/error.lua` (mirrors `git/error.lua`'s `value, err` shape; kinds `not_installed`, `not_authenticated`, `forbidden`, `rate_limited`, `not_found`, `no_remote`, `bad_remote`, `api_error`, `invalid`, `request_failed`, `spawn_failed`, `timeout`), `github/cmd.lua` (`request`/`graphql`/`classify`, an explicit `gh` env allow-list via `core/job.lua`'s existing `env = {name = false}` mechanism, `gh api -i` to expose `Retry-After`), `github/host.lua` (host/owner/repo resolution: `config.github.host` > `GH_HOST` > the `origin` remote's parsed host, since `gh api --hostname` does not do `gh pr view`'s repo-context inference), `github/auth.lua` (`gh auth status --json hosts`, always exit 0 in JSON mode, so a rejected/expired token is distinguishable from never-logged-in; `health.lua`'s warn-level gh check now delegates here instead of its own text-parsing), `github/pr.lua` (`fetch(repo, number)` — one static GraphQL query for node id, SHAs, base/head refs, fork ownership; never touches `/pulls/{n}/files`). Test double `tests/ghstub.lua` (a generated shell script standing in for `gh`, the `gh` analogue of `tests/gitrepo.lua`) exercises the real subprocess boundary with no network.

Verified live against the real API (authenticated local `gh` 2.101.0) before and after coding: success/error body shapes for GraphQL and REST-style failures, `pr.fetch` end-to-end against `cli/cli#1`. `X-Github-Next-Global-ID: 1` sent on every GraphQL call; variables only via `-f`/`-F`, never `string.format`-built query text. Scope stops at the client — worktree checkout, viewed marks, comment threads, resolve and review verdict are still open steps that build on this.

### result: implement: wire the conflict view in

Merged to `main` in `93e0da3` (branch `worktree-agent-ac2d27511e0afd0ce`, commit `fc8d79a`), run in parallel with the steps above and below; its overlap with the range-compare/line-history branch on `plugin/nvim-diff.lua` and `tests/spec/views_diff_spec.lua` was a clean textual (additive) merge conflict, resolved by keeping both sides — no logic fixup needed. `make check` on the branch: 401/401 (12 new); 452/452 after the full three-way merge.

Added `:NvimDiffConflict [path]` (`views/conflict.lua` gained `M.command`, registered in `plugin/nvim-diff.lua` next to the other commands). `views/diff.lua`'s `View:select` now routes a `U`-status entry to a new `View:open_conflict(entry)`, opening the conflict view in its own tab and leaving a note in the diff view's area instead of a pair; the panel's own `U` entries, in panel order, become the conflict view's file list. `views/conflict.lua` gained `next_file`/`prev_file` on `keymaps.view.next_file`/`prev_file` (`<Tab>`/`<S-Tab>`, reused rather than a new `keymaps.conflict` entry), stepping panes **in place** (`M.open`'s setup was split into `View:load()`, shared by `M.open` and the step path, so an existing `view` reference stays valid across a step).

Found: a conflicted file only surfaces as `U` through a diff that consults the index — `git diff <commit>` (the default `:NvimDiffOpen`, no `--cached`) never reports `U`, just a plain `M` with the markers embedded as content; only `--cached`/`--staged` or an index-vs-worktree diff shows it. So today a user hitting a conflict needs `--cached`/`--staged` or `:NvimDiffConflict` directly to see it flagged — ties into the existing open question about a staged/unstaged split for `:NvimDiffOpen`'s default. Out of scope, confirmed untouched: "resolve all" keys, a stage-the-result key, context folding in the three top panes.

### result: implement: range compare and line history

Merged to `main` in `1ee828c` (branch `worktree-agent-aacc9d114c02be4ff`, commits `7985344` + `af205ab`), run in parallel with the two steps above. `make check` on the branch: 405/405 (12 new); 452/452 after the full three-way merge (its overlap with the conflict-view branch on `plugin/nvim-diff.lua`/`config.lua`/`views/diff.lua`/`tests/spec/views_diff_spec.lua` was additive and merged clean).
